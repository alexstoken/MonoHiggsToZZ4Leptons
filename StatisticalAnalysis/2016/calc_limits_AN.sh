...
if [ $1 == "4l" ]; then
  python MonoHiggsSelection.py --channel 4mu   >> yields.txt
  python MonoHiggsSelection.py --channel 4e    >> yields.txt
  python MonoHiggsSelection.py --channel 2e2mu >> yields.txt
else python MonoHiggsSelection.py --channel $1 >> yields.txt
fi
