...
def get_data(t):
  ...
  return (weight, pfmet, mass4l, mT, dphi, Dkin, D)
...
def get_punzi(signal, flist, cut_pfmet):
  ...
  for f in flist:
    ...
    weight, pfmet, mass4l, m4lt, dphi, Dkin, D = get_data(t)
...
if __name__ == "__main__":
  ...
  if '2015' in flist[0]:
    punzi600,  eff_S600  = get_punzi('MZP600_MA0300', flist, cut_pfmet)
    punzi800,  eff_S800  = get_punzi('MZP800_MA0300', flist, cut_pfmet)
    punzi1000, eff_S1000 = get_punzi('MZP1000_MA0300', flist, cut_pfmet)
    punzi1200, eff_S1200 = get_punzi('MZP1200_MA0300', flist, cut_pfmet)
    punzi1400, eff_S1400 = get_punzi('MZP1400_MA0300', flist, cut_pfmet)
    punzi1700, eff_S1700 = get_punzi('MZP1700_MA0300', flist, cut_pfmet)
    punzi2000, eff_S2000 = get_punzi('MZP2000_MA0300', flist, cut_pfmet)
    punzi2500, eff_S2500 = get_punzi('MZP2500_MA0300', flist, cut_pfmet)
  if '2016' in flist[0]:
  ...
  print 'MET cut [GeV]  | %3.f  | %3.f  | %3.f  | %3.f  | %3.f  | %3.f  | %3.f  | %3.f' % (cut_pfmet[np.argmax(punzi600)], cut_pfmet[np.argmax(punzi800)], cut_pfmet[np.argmax(punzi1000)], cut_pfmet[np.argmax(punzi1200)], cut_pfmet[np.argmax(punzi1400)], cut_pfmet[np.argmax(punzi1700)], cut_pfmet[np.argmax(punzi2000)], cut_pfmet[np.argmax(punzi2500)])
